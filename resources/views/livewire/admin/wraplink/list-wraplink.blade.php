<div class="rounded-sm border border-stroke bg-white shadow-default dark:border-strokedark dark:bg-boxdark">
    <div class="px-4 py-6 md:px-6 xl:px-7.5">
        <div class="flex justify-between items-center">
            <h4 class="text-xl font-bold text-black dark:text-white inline">DANH SÁCH BỌC LINK</h4>
            <a href="{{route('admin.wraplinks.add')}}" class="inline-flex items-center px-4 py-2 bg-blue-500 border border-transparent rounded-md font-semibold text-xs text-white uppercase tracking-widest hover:bg-blue-600 active:bg-blue-700 focus:outline-none focus:border-blue-900 focus:ring ring-blue-300 disabled:opacity-25 transition ease-in-out duration-150">
                Thêm mới
            </a>
        </div>
    </div>
    ...
    <div class="overflow-x-auto">
        <table class="min-w-full divide-y divide-gray-200">
            <thead class="bg-gray-200">
                <tr>
                    <th scope="col" class="px-2 py-4 text-sm font-medium text-gray-700 uppercase tracking-wider w-10 text-center">
                        <svg viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg" onclick="checkDeleteMultiple('cbx_delete_wraplink');" class="cursor-pointer">
            ...
            <tbody class="bg-white divide-y divide-gray-200 text-sm">
                @if ($wraplinks->isEmpty())
                    <tr>
                        <td class="px-2 py-2 whitespace-nowrap text-center" colspan="7">Không có dữ liệu</td>
                    </tr>
                @endif
                @foreach ($wraplinks as $index => $wraplink)
                    <tr>
                        <td class="px-2 py-2 whitespace-nowrap text-center">
                            <input type="checkbox" name="cbx_delete_wraplink"  class="cursor-pointer w-4 h-4 text-blue-600 bg-gray-100 border-gray-300 rounded focus:ring-blue-500 dark:focus:ring-blue-600 dark:ring-offset-gray-800 focus:ring-2 dark:bg-gray-700 dark:border-gray-600" wire:model="selected_index.{{$index}}" value="{{$wraplink->id}}">
                        </td>
                        <td class="px-2 py-2 whitespace-nowrap text-center">{{ $wraplinks->perPage() * ($wraplinks->currentPage() - 1) + $loop->iteration }}</td>
                        <td class="px-2 py-2 whitespace-nowrap text-center">
                            @if ($wraplink->logo)
                                <img src="{{ asset('storage/images/wraplinks/' . $wraplink->logo) }}" alt="wraplink Logo" class="w-15 h-15 shadow-md">
                            @else
                                <img src="{{ asset('library/images/image-not-found.jpg') }}" alt="wraplink Logo" class="w-15 h-15 shadow-md">
                            @endif
                        </td>
                        <td class="px-2 py-2 whitespace-nowrap" title="{{$wraplink->name}}">
                                {{ strlen($wraplink->name) > 20 ? substr($wraplink->name, 0, 20) . '...' : $wraplink->name }}
                        </td>
                        <td class="px-2 py-2 whitespace-nowrap">
                                ...
                                <input type="hidden" class="full-link" value="{{ $fullLink }}">

                                <span class="copy-icon cursor-pointer" title="Copy Đường Dẫn" onclick="copyLink(this)">
                        ...
                        <td class="px-2 py-2 whitespace-nowrap text-center">
                            ...
                            <a data-modal-target="popup-delete-item" data-modal-toggle="popup-delete-item" onclick="parseDataDelete('{{$wraplink->id}}', '{{$wraplink->name}}')" class="inline-flex items-center text-red-600 hover:text-red-900 cursor-pointer">
    ...
    <div class="px-4 py-6 md:px-6 xl:px-7.5">
        {{$wraplinks->links('livewire.custom-pagination')}}
    </div>
